...
\maketitle
\begin{abstract}
Word embeddings algorithms have the ability to learn word representation from unlabelled data.
...
We also found that when using word embeddings, only several hundred of training instances are needed to reach a decent performance. 
Surprising, we could not find any leading word embedding method across the different tasks and proposed settings. Nevertheless, word embeddings are always helping to improve the performance and are especially useful for task or domains with limited training data. They are also easy to train and to integrate to existing NLP systems, and therefore, should be considered in modern NLP architectures.
\end{abstract}
