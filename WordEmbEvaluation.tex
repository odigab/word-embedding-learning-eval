...
\nss{See also work by Mo Yu (\url{http://www.asiteof.me/yumo}) and Manaal Faruqui (\url{http://www.cs.cmu.edu/~mfaruqui/pubs.html}).}
...
%631 million words wikipedia. 221 million words
...
They evaluate Brown clusters, and word embeddings from \newcite{Collobert2008} 
and \newcite{Mnih2008}. Words representations were learned using the same data set (a sub-sample of the of the RCV1 corpus), of 37 millions words and a vocabulary size of 269K.
...
%they set/discovered the default (best) scaling parameter, which turn to be: 0.1
...
%They were trained for 50 epochs, not just 20 epochs (In comparison to Turian et al. (2009)); We initialized all embedding dimensions uniformly in the range

\newcite{owoputi2013improved} demonstrated that Brown clusters enhance Twitter POS tagging. 
...
\newcite{Schneider+:2014} presented a MWE analyzer that, among other features,  used unsupervised word clusters. 
...
%( they considered proper names as MWE)
