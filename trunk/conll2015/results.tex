...
\begin{subfigure}{7cm}
	\centering
    \includegraphics[scale=0.42]{plots/map-pos-color-invert}    	
	\subcaption{\pos (\accuracy)}	
	\label{pos}
\end{subfigure}
\begin{subfigure}{7cm}
	\centering
    \includegraphics[scale=0.42]{plots/map-chunk-color-invert}
...
\begin{subfigure}{7cm}
	\centering
    \includegraphics[scale=0.42]{plots/map-ner-color-invert}    	
	\subcaption{\ner (\fscore)}	
	\label{ner}
\end{subfigure}
\begin{subfigure}{7cm}
	\centering
    \includegraphics[scale=0.42]{plots/map-mwe-color-invert}
...
\paragraph{\RQ[1]: Are the selected word embeddings better than one-hot unigram features
  and Brown clusters?}
As shown in \tabref{benchmark}, the
...
would appear to be: yes, word embeddings are better than unigrams when
there is little training data, but they are not markedly better than
Brown clusters.
...
For example, %\CW and 
\Glove performs substantially worse than \Skipgram
for both \pos and \ner without updating, but \emph{with} updating, the
relative empirical gap between the best performing method becomes
...
vector fields generated using matplotlib and t-SNE \cite{vanderMaaten:Hinton:2008}. Half
...
\url{https://goo.gl/Y8bk2w}.  In each plot, a single arrow
