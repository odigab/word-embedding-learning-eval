...
walkDirChuB = sys.argv[4]
walkDirMwe = sys.argv[5]
...
alist_filter = ['brown_cluster_v2000_best.txt', 
	...
	'skip_gram_negsam_noupdated_best.txt', 
	'skip_gram_negsam_updated_incretuneparam.txt']
for root, subdirs, files in sortedWalk(walkDirChuB):
	#print('--\nroot = ' + root)
	for subdir in sorted(subdirs):
		print('\t- subdirectory ' + subdir)		
	for filename in files:  		  		
		if filename in alist_filter:
			...
			with open(filePath, 'rb') as f:
				...
				for line in lines[1:]: 
					p = line.split()	
					x.append(float(p[0]))				
					z.append(float(p[7]))  # BROWN_out-of-vocabulary_Accuracy
				# convert to numpy array
				#print('x ', x)			
				#print('z', z) 
				xv = np.array(x)
				...
				plt.ylim([.5,1])
				plt.xlabel('Training size')
				plt.title('Chunking accuracy for out-of-domain OOV')
				x = []
				z = []
				xv = []
				zv = []
...
# loc= 'center right', bbox_to_anchor=(0, -0.2),fancybox=True, shadow=True, ncol=2)

fig.tight_layout()
savefig(sys.argv[6], bbox_inches='tight')
